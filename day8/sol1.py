...
closest_points = np.argsort(linearized)[:MAX_LINKS]
...
sorted_boxes = sorted(boxes, key=lambda x: -len(x))
total = list(map(lambda x: len(x), sorted_boxes[:3]))
print("sorted_boxes", np.prod(total))
